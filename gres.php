...
{
    $r = db_query(
        'SELECT * FROM users WHERE '.mysql_escape_string($by).' LIKE \''.mysql_escape_string($val).'\' LIMIT 1'
    );
...
function reloadsperre_iCheckIP($cur_ip, $save)
{ // ----------------- iCheckIP ----------------
...
function reloadsperre_CheckIP($save)
{ // ------------------ reloadsperre_CheckIP ---------------
    global $lock_ip;
    $ip = GetIP();
    $ip = $ip['ip'];
    if ($lock_ip == 0) {
        return true;
    } elseif (reloadsperre_iCheckIP($ip, $save) == 0) {
        return true;
    } else {
        return false;
    }
    ...
    */
    switch ($id) {
        ...
        case 'lan':
            ...
            break;
        case 'fw':
            $b = ($pc['cpu'] >= 6 && $pc['ram'] >= 2 && $pc[$id] < itemmaxval($id));
            break;
        case 'mk':
            $b = ($pc['cpu'] >= 12 && $pc['sdk'] >= 3 && $pc[$id] < itemmaxval($id));
            break;
        case 'av':
            $b = ($pc['cpu'] >= 10 && $pc['ram'] >= 3 && $pc[$id] < itemmaxval($id));
            break;
        case 'sdk':
            $b = ($pc['cpu'] >= 8 && $pc['ram'] >= 2 && $pc[$id] < itemmaxval($id));
            break;
        case 'ips':
            $b = ($pc['cpu'] >= 8 && $pc['sdk'] >= 2 && $pc[$id] < itemmaxval($id));
            break;
        case 'ids':
            $b = ($pc['cpu'] >= 15 && $pc['sdk'] >= 3 && $pc[$id] < itemmaxval($id));
            break;
        case 'trojan':
            $b = ($pc['mk'] >= 4 && $pc[$id] < itemmaxval($id) && $pc['ram'] >= 4);
            break;
        case 'rh':
            $b = ($pc['cpu'] >= 18 && $pc['ram'] >= 7 && $pc['sdk'] >= 5 && $pc['mk'] >= 10 && $pc[$id] < itemmaxval(
                    $id
                ));
            break;
        default:
